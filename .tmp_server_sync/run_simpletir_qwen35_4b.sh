...
# Validation Parquet list, colon-separated (e.g. "val100.parquet:aime24.parquet:aime25.parquet").
# Each file is validated independently and reported under its data_source prefix.
...
# wandb monitoring (USE_WANDB=1 adds 'wandb' to trainer.logger).  The API key
# must already be deployed (WANDB_API_KEY env or `wandb login`); we refuse to
# start otherwise because wandb.init would block on an interactive prompt.
USE_WANDB="${USE_WANDB:-0}"
# Number of random validation generations surfaced as a wandb table each round.
LOG_VAL_GENERATIONS="${LOG_VAL_GENERATIONS:-24}"
# Every validation round dumps the FULL response set (input/output/gt/score)
# as one JSONL per step here, for offline "still makes sense" inspection.
...
[[ -f "${TRAIN_FILE}" ]] || { echo "missing train Parquet: ${TRAIN_FILE}" >&2; exit 2; }
IFS=':' read -r -a VAL_FILE_LIST <<< "${VAL_FILE}"
for _vf in "${VAL_FILE_LIST[@]}"; do
  [[ -f "${_vf}" ]] || { echo "missing validation Parquet: ${_vf}" >&2; exit 2; }
done
...
PYCHECK
  then
    echo "USE_WANDB=1 but no wandb credential found (set WANDB_API_KEY or run: ${PYTHON_BIN} -m wandb login)" >&2
    exit 2
  fi
fi
...
if [[ -n "${PROBE_VAL_FILE}" ]]; then
  VAL_FILES_HYDRA="${VAL_FILES_HYDRA%,]},'${PROBE_VAL_FILE}']"
fi
if [[ "${USE_WANDB}" == "1" ]]; then
...
ARGS=(
  ...
  "trainer.validation_data_dir=${VAL_DUMP_DIR:-${RUN_DIR}/val_generations}"
